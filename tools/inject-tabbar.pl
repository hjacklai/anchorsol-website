#!/usr/bin/env perl
# Inject bottom-tabbar HTML into all /learn/*.html pages, just before
# `<script src="../js/site.js" defer></script>`. Idempotent.
use strict;
use warnings;
...
my ($touched, $skipped, $missing) = (0, 0, 0);
for my $f (@files) {
  my $fp = "$dir/$f";
  ...
  if ($html =~ /class="tabbar"/) {
    $skipped++;
    print "SKIP  $f (already has tabbar)\n";
    next;
  }
...
print "\nDone. $touched updated, $skipped already had tabbar, $missing missing anchor.\n";
