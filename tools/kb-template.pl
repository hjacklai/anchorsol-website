#!/usr/bin/env perl
# Assemble a KB article from a config + body file.
# Usage: perl tools/kb-template.pl <config.txt> <body.html> <output.html>
#
# Config file format (one key=value per line):
#   slug=mse-wall-history
#   title=MSE Wall History: From Henri Vidal 1963 to Modern Practice
#   description=...
#   keywords=...
#   tag=History reference
#   h1=The history of MSE walls...
#   lede=<p>...</p>
...
die "Usage: $0 <config> <body> <output>\n" unless @ARGV == 3;
my ($config_file, $body_file, $output_file) = @ARGV;

# Parse config
my %c;
open my $cfg, '<:utf8', $config_file or die "config: $!";
while (my $line = <$cfg>) {
  chomp $line;
...
# Read body
open my $bf, '<:utf8', $body_file or die "body: $!";
my $body = do { local $/; <$bf> };
close $bf;

# Defaults
$c{image} //= 'https://anchorsolwall.com/assets/videos/wall-linear-poster.jpg';
$c{wordcount} //= 2200;
$c{minutes} //= 10;

my $url = "https://anchorsolwall.com/learn/$c{slug}.html";
...
<script type="application/ld+json">
{
  ...
  "\@graph": [
    {"\@type": "Article", "\@id": "$url#article",
     ...
     "wordCount": $c{wordcount}, "inLanguage": "en-MY"},
...
<nav class="breadcrumb" aria-label="Breadcrumb">
  <a href="../">Home</a> · <a href="./">Knowledge Base</a> · <span>$c{breadcrumb}</span>
</nav>

<article>
<header class="article__head">
  <span class="article__tag">$c{tag}</span>
  <h1>$c{h1}</h1>
  $c{lede}
  <p class="article__meta">By <b>Dr. Ir. Lai Yip Poon</b>, Founder & Chief Designer · AnchorSOL® Wall Sdn. Bhd. · ~$c{minutes} min read</p>
</header>
...
<aside class="article__related">
  ...
  <div class="article__related-grid">
    <a class="kb-card" href="$c{related1href}">
      <span class="kb-card__tag">$c{related1tag}</span>
      <h3>$c{related1title}</h3>
      <p>$c{related1desc}</p>
      <span class="kb-card__cta">Read guide →</span>
    </a>
    <a class="kb-card" href="$c{related2href}">
      <span class="kb-card__tag">$c{related2tag}</span>
      <h3>$c{related2title}</h3>
      <p>$c{related2desc}</p>
      <span class="kb-card__cta">Read guide →</span>
    </a>
    <a class="kb-card" href="$c{related3href}">
      <span class="kb-card__tag">$c{related3tag}</span>
      <h3>$c{related3title}</h3>
      <p>$c{related3desc}</p>
      <span class="kb-card__cta">Read guide →</span>
    </a>
  </div>
...
open my $out, '>:utf8', $output_file or die "output: $!";
print $out $html;
close $out;
